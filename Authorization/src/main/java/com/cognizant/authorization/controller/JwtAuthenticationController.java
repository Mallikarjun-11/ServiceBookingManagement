...
import com.cognizant.authorization.model.AllAppUsers;
...
import com.cognizant.authorization.service.allservice;
...
public class JwtAuthenticationController {
	...
	public ResponseEntity<UserToken> login(@RequestBody UserLoginCredential userlogincredentials) throws Exception {
		...
		authenticate(userlogincredentials.getName(), userlogincredentials.getPassword());
		
		
		final UserDetails userdetails = service.loadUserByName(userlogincredentials.getName());
		
		
		return new ResponseEntity<>(new UserToken(userlogincredentials.getName(), jwtutil.generateToken(userdetails)),
				HttpStatus.OK);
	}
	...
	public String healthCheck() {
			/*AppUser appUser=restTemplate.getForObject("http://localhost:9095/user", AppUser.class);
		...
		for (AppUser appUser : allAppUser.getAllAppUser()) {
			allUserRepository.save(appUser);
		}
		return "Auth Microservice is Active";
